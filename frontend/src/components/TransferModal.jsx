...
function TransferModal({ onClose, onSuccess, children, parentBalance }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await walletAPI.transfer(childId, parseFloat(amount));
                                ...
                                >
                                    ...
                                    {children.map((child) => (
                                        <option key={child._id} value={child._id}>
                                            {child.username} (Balance: {(child.balance || 0).toFixed(2)} EGP)
                                        </option>
                                    ))}
                                </select>
                            </div>
                                ...
                                <div className="card" style={{ background: 'var(--success-gradient)', padding: '1rem', marginBottom: '1rem' }}>
                                    <div style={{ color: 'white', textAlign: 'center' }}>
                                        ...
                                        <div style={{ fontSize: '1.5rem', fontWeight: 700 }}>
                                            {((selectedChild.balance || 0) + parseFloat(amount)).toFixed(2)} EGP
                                        </div>
                                    </div>
                                </div>
                            )}
